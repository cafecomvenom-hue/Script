...
--------------------------------------------------------------------------------
-- FLY GOD MODE + WORLD NOCLIP SYSTEM
--------------------------------------------------------------------------------

-- Palavras-chave para detectar objetos perigosos (tsunami, ondas, projéteis, etc.)
local DangerKeywords = {
    "tsunami", "wave", "flood", "water", "lava", "fire", "explosion",
    "projectile", "bullet", "arrow", "missile", "bomb", "trap",
    "hazard", "danger", "kill", "death", "damage", "hurt",
    "poison", "acid", "spike", "laser", "beam", "blast",
    "storm", "tornado", "lightning", "meteor", "rock", "boulder",
    "debris", "falling", "crush", "smash", "hit", "attack",
    "surge", "tide", "current", "flow", "splash", "crash",
    "onda", "mar", "oceano", "tempestade", "destrui"
}

-- Verificar se um nome contém alguma palavra perigosa
local function IsDangerousName(name)
    local lowerName = string.lower(name)
    for _, keyword in pairs(DangerKeywords) do
...
-- Neutralizar uma parte perigosa para o jogador
...
-- Escanear e neutralizar todo o workspace
local function ScanAndNeutralizeWorld()
    pcall(function()
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") then
                if IsDangerousName(obj.Name) or
                   (obj.Parent and IsDangerousName(obj.Parent.Name)) then
                    NeutralizePart(obj)
                end
            end
        end
...
-- Monitorar objetos novos no workspace
local function StartWorldMonitor()
    -- Desconectar monitores anteriores
    ...
    -- Escanear tudo que já existe
    ScanAndNeutralizeWorld()
        ...
        task.defer(function()
            pcall(function()
                if obj:IsA("BasePart") then
                    ...
                    -- Interceptar qualquer Touched no personagem
                    ...
                    if char then
                        local conn = obj.Touched:Connect(function(hit)
                            ...
                            if hit and hit:IsDescendantOf(char) then
                                ...
                                -- Restaurar vida se tiver sido reduzida
                ...
                elseif obj:IsA("Model") then
                    if IsDangerousName(obj.Name) then
                        NeutralizeModel(obj)
                    end
                end
            end)
...
-- Parar monitoramento do mundo
local function StopWorldMonitor()
    if WorldNoclipConnection then
        pcall(function() WorldNoclipConnection:Disconnect() end)
        WorldNoclipConnection = nil
    end
...
local function EnableFlyGodMode()
    ...
    -- Desabilitar colisão no personagem
    ...
    -- Monitorar partes novas adicionadas ao personagem
    ...
    -- Desabilitar todos os estados perigosos do Humanoid
    pcall(function()
    ...
    -- Iniciar monitoramento do mundo (tsunami, ondas, etc.)
    StartWorldMonitor()
end
...
local function StopFly()
    ...
    -- Desativar God Mode + World Noclip
    DisableFlyGodMode()
...
local FlightTog = MainTab:AddToggle("Fly Hack", "fly_hack", function(v)
    ...
    if v then
        if IsAndroid then
            Xan.Notify({
                Title = "Flight",
                Content = "Joystick mover + ▲/▼ subir/descer | God Mode + World Noclip ON",
                Type = "Success"
            })
        else
            Xan.Notify({
                Title = "Flight",
                Content = "WASD mover, E/Space subir, Q/Shift descer | God Mode + World Noclip ON",
                Type = "Success"
            })
        end
    else
        Xan.Notify({
            Title = "Flight",
            Content = "Fly desativado | God Mode + World Noclip OFF",
            Type = "Info"
        })
    end
end)
...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    if Config.Fly then
        ...
        -- GOD MODE: Forçar vida máxima a cada frame
        if hum and hum.Health > 0 then
            hum.Health = hum.MaxHealth
        end

        -- FULL NOCLIP: Personagem sem colisão e sem toque
        for _, part in pairs(char:GetDescendants()) do
            if part:IsA("BasePart") then
                part.CanCollide = false
                part.CanTouch = false
        ...
        -- Garantir ForceField invisível existe
        if not char:FindFirstChild("FlyForceField") then
            pcall(function()
        ...
        pcall(function()
            ...
            for _, obj in pairs(workspace:GetDescendants()) do
                if obj:IsA("BasePart") and not obj:IsDescendantOf(char) then
                    ...
                    -- Neutralizar objetos em movimento próximos (projéteis, ondas)
                    local distance = (obj.Position - rootPos).Magnitude
                    if distance < scanRadius then
                        local velocity = obj.AssemblyLinearVelocity or obj.Velocity
                        if velocity and velocity.Magnitude > 20 then
                            obj.CanCollide = false
                            obj.CanTouch = false
                        end

                        -- Objetos muito grandes se movendo (tsunamis)
                        if obj.Size.Magnitude > 50 and velocity and velocity.Magnitude > 5 then
                            obj.CanCollide = false
                            obj.CanTouch = false
                        end
                    end
        ...
        -- Anular velocidades impostas no personagem (empurrões de ondas/explosões)
        if root then
