...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    --// FLY HACK (Desktop + Android compatible)
    if Config.Fly then
        ...
        if not IsAndroid then
            -- DESKTOP: WASD + E/Q for vertical control
            ...
            -- ANDROID: Use touch input only (don't use Humanoid.MoveDirection to avoid auto-flying)
            local touchPositions = UserInputService:GetTouchPositions()
            if #touchPositions > 0 then
                local avgX, avgY = 0, 0
                for _, touchPos in pairs(touchPositions) do
                    avgX = avgX + touchPos.X
                    avgY = avgY + touchPos.Y
                end
                avgX = avgX / #touchPositions
                avgY = avgY / #touchPositions
                
                -- Map touch position to virtual stick centered at screen center
                local screenSize = Camera.ViewportSize
                local centerX = screenSize.X / 2
                local centerY = screenSize.Y / 2
                
                local touchX = (avgX - centerX) / centerX
                local touchY = (centerY - avgY) / centerY
                
                local magnitude = math.sqrt(touchX * touchX + touchY * touchY)
                
                -- Apply deadzone (10%) for stability
                if magnitude > 0.1 then
                    local normX = math.min(1, touchX / magnitude)
                    local normY = math.min(1, touchY / magnitude)
                    
                    moveDirection = moveDirection + (Camera.CFrame.RightVector * normX + Camera.CFrame.LookVector * normY)
                end
            end
            
            -- Vertical control: use up/down button flags (set by button handlers)
