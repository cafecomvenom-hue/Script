...
local FlightTog = MainTab:AddToggle("Fly Hack", "fly_hack", function(v)
    ...
    if IsAndroid then
        ...
        if v then
            Xan.Notify({Title = "Flight", Content = "Toque a tela para se mover, use botões para subir/descer", Type = "Info"})
        end
    end
...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    --// FLY HACK (Desktop + Android fully compatible)
    if Config.Fly then
        ...
        if not IsAndroid then
            ...
            -- ===== MOBILE CONTROLS =====
            -- Primary Method: Native Humanoid movement (analog stick / virtual pad)
            if hum and hum.MoveDirection.Magnitude > 0 then
                -- Use the native analog stick input
                local moveDir = hum.MoveDirection
                moveDirection = moveDirection + (Camera.CFrame.RightVector * moveDir.X + Camera.CFrame.LookVector * moveDir.Z)
            else
                -- Fallback: interpret touch positions as movement
                local touchPositions = UserInputService:GetTouchPositions()
                
                if #touchPositions > 0 then
                    -- Find the leftmost touch (movement input area)
                    local leftmostTouch = nil
                    local minX = math.huge
                    
                    for _, touchPos in pairs(touchPositions) do
                        if touchPos.X < minX then
                            minX = touchPos.X
                            leftmostTouch = touchPos
                        end
                    end
                    
                    if leftmostTouch then
                        -- Map touch position to virtual stick (left side of screen)
                        local screenSize = Camera.ViewportSize
                        local stickCenterX = screenSize.X * 0.15 -- Left side center
                        local stickCenterY = screenSize.Y * 0.85 -- Bottom area
                        local stickRadius = screenSize.X * 0.12
                        
                        local touchX = leftmostTouch.X - stickCenterX
                        local touchY = stickCenterY - leftmostTouch.Y
                        
                        local magnitude = math.sqrt(touchX * touchX + touchY * touchY)
                        
                        -- Apply deadzone (15%)
                        if magnitude > (stickRadius * 0.15) then
                            local normX = math.min(1, touchX / magnitude)
                            local normY = math.min(1, touchY / magnitude)
                            
                            -- Apply sensitivity curve
                            local sensitivity = magnitude / stickRadius
                            if sensitivity > 1 then sensitivity = 1 end
                            
                            moveDirection = moveDirection + ((Camera.CFrame.RightVector * normX + Camera.CFrame.LookVector * normY) * sensitivity)
                        end
                    end
                end
            end
            
            -- Vertical Control: Up/Down Buttons or Jump Input
            if AndroidUpPressed or AndroidJumpPressed then
                verticalInput = verticalInput + 1
            end
