...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    if Config.Fly then
        ...
        if not IsAndroid then
            ...
            -- Removed A/D for consistency with Android
            ...
            -- ANDROID: NEW APPROACH - Use Humanoid.MoveDirection but ONLY Z axis
            if hum and hum.MoveDirection.Magnitude > 0 then
                local moveDir = hum.MoveDirection
                -- CRITICAL: Only use Z component (forward/backward)
                -- Completely ignore X component (left/right)
                local forwardInput = moveDir.Z
                
                if math.abs(forwardInput) > 0.1 then -- Deadzone
                    moveDirection = Camera.CFrame.LookVector * forwardInput
                end
            end
