...
local function CreateFlyUI()
    ...
    MainFrame.Position = UDim2.new(1, -160, 0.5, -115)
    ...
    Divider.Position = UDim2.new(0, 10, 0, 80)
    ...
    FlyUpButton.Position = UDim2.new(0, 15, 0, 90)
    ...
    FlyDownButton.Position = UDim2.new(0, 15, 0, 155)
...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    if Config.Fly then
        ...
        if not IsAndroid then
            ...
            -- ANDROID: Separate horizontal (joystick) and vertical (buttons) completely
            
            -- HORIZONTAL MOVEMENT: Only from joystick Z axis (forward/backward)
            if hum and hum.MoveDirection.Magnitude > 0 then
                local moveDir = hum.MoveDirection
                -- Only use Z component (forward/backward), ignore X (left/right)
                local forwardInput = moveDir.Z
                
                if math.abs(forwardInput) > 0.1 then -- Deadzone
                    -- Movement is ONLY in the forward/backward direction relative to camera
                    horizontalDirection = Camera.CFrame.LookVector * forwardInput
                end
            end
            ...
            -- If neither button is pressed, verticalDirection stays at zero (maintains altitude)
        end
