...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    if Config.Fly then
        ...
        if not IsAndroid then
            ...
            -- ANDROID: Movimento automático APENAS para frente
            -- Sempre vai para frente na direção da câmera quando Fly está ativo
            
            -- Get camera's look direction (na horizontal)
            local lookVector = Camera.CFrame.LookVector
            local horizontalLook = Vector3.new(lookVector.X, 0, lookVector.Z)
            
            if horizontalLook.Magnitude > 0 then
                horizontalLook = horizontalLook.Unit
                horizontalDirection = horizontalLook -- Sempre para frente
            end
            ...
            if FlyUIContainer and FlyUIContainer.MainFrame then
                ...
                if joystickLabel then
                    joystickLabel.Text = "Fly: FORWARD [Auto]"
                    joystickLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
                end
            end
            
            -- VERTICAL MOVEMENT: Apenas pelos botões UP/DOWN
