...
local AndroidJumpPressed = false

-- FLY UI ELEMENTS
local FlyUIContainer = nil
...
local function SafeTeleport(targetCFrame, offset)
    ...
    if offset then
        finalCFrame = targetCFrame * offset
    end
    
    -- Add small upward offset to prevent clipping into ground (3 studs up)
    finalCFrame = finalCFrame * CFrame.new(0, 3, 0)
    
    -- Temporarily disable death on collision
    local originalHealth = humanoid.Health
    local originalMaxHealth = humanoid.MaxHealth
    
    -- Method 1: Use PivotTo (most reliable for R15/R6)
    pcall(function()
        LocalPlayer.Character:PivotTo(finalCFrame)
    end)
    
    -- Small delay to let physics settle
    task.wait(0.1)
    
    -- Restore health if damaged during teleport
...
local function CreateAndroidButtons()
    ...
    local buttonConfigs = {
        ...
        {Name = "UpBtn", Pos = UDim2.new(0.9, -60, 0.35, -30), Color = Color3.fromRGB(100, 200, 100), Hidden = true, Size = 60},
        {Name = "DownBtn", Pos = UDim2.new(0.9, -60, 0.55, -30), Color = Color3.fromRGB(200, 100, 100), Hidden = true, Size = 60},
    }
...
--// CREATE FLY UI FOR ANDROID (Creates immediately, shows/hides based on Fly state)
local function CreateFlyUI()
    if not IsAndroid then 
        print("[DEVS HUB] Not Android, skipping Fly UI creation")
        return 
    end
    
    print("[DEVS HUB] Creating Fly UI for Android...")
    
    -- Clean up old UI if exists
    local oldUI = game:GetService("CoreGui"):FindFirstChild("FlyUI")
    if oldUI then
        oldUI:Destroy()
        print("[DEVS HUB] Removed old Fly UI")
    end
    
    -- Create main container
    FlyUIContainer = Instance.new("ScreenGui")
    FlyUIContainer.Name = "FlyUI"
    FlyUIContainer.ResetOnSpawn = false
    FlyUIContainer.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    FlyUIContainer.IgnoreGuiInset = true
    
    local success = pcall(function()
        FlyUIContainer.Parent = game:GetService("CoreGui")
    end)
    
    if not success then
        FlyUIContainer.Parent = UserGui
        print("[DEVS HUB] Fly UI created in PlayerGui (CoreGui blocked)")
    else
        print("[DEVS HUB] Fly UI created in CoreGui")
    end
    ...
    MainFrame.Position = UDim2.new(1, -160, 0.5, -120)
    MainFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    MainFrame.BackgroundTransparency = 0.1
    MainFrame.BorderSizePixel = 0
    MainFrame.Visible = false -- Hidden by default
    MainFrame.ZIndex = 100
    MainFrame.Parent = FlyUIContainer
    ...
    local JoystickStatus = Instance.new("TextLabel")
    JoystickStatus.Name = "JoystickStatus"
    JoystickStatus.Size = UDim2.new(1, -20, 0, 18)
    JoystickStatus.Position = UDim2.new(0, 10, 0, 68)
    JoystickStatus.BackgroundTransparency = 1
    JoystickStatus.Text = "Joystick: 0.00"
    JoystickStatus.TextColor3 = Color3.fromRGB(100, 255, 100)
    JoystickStatus.TextSize = 11
    JoystickStatus.Font = Enum.Font.GothamMedium
    JoystickStatus.ZIndex = 101
    JoystickStatus.Parent = MainFrame
    ...
    FlyUpButton.Name = "UpButton"
    FlyUpButton.Size = UDim2.new(1, -30, 0, 55)
    FlyUpButton.Position = UDim2.new(0, 15, 0, 102)
    FlyUpButton.BackgroundColor3 = Color3.fromRGB(50, 200, 100)
    ...
    UpStroke.Color = Color3.fromRGB(100, 255, 150)
    UpStroke.Thickness = 2
    UpStroke.Parent = FlyUpButton
    
    -- DOWN BUTTON
    FlyDownButton = Instance.new("TextButton")
    FlyDownButton.Name = "DownButton"
    FlyDownButton.Size = UDim2.new(1, -30, 0, 55)
    FlyDownButton.Position = UDim2.new(0, 15, 0, 167)
    FlyDownButton.BackgroundColor3 = Color3.fromRGB(255, 100, 50)
    ...
    DownStroke.Parent = FlyDownButton
    
    -- Button events
    FlyUpButton.MouseButton1Down:Connect(function()
        AndroidUpPressed = true
        FlyUpButton.BackgroundColor3 = Color3.fromRGB(80, 255, 140)
        FlyUpButton.Size = UDim2.new(1, -32, 0, 53)
        print("[DEVS HUB] UP button pressed")
    end)
    ...
    FlyUpButton.MouseButton1Up:Connect(function()
        ...
        FlyUpButton.BackgroundColor3 = Color3.fromRGB(50, 200, 100)
        FlyUpButton.Size = UDim2.new(1, -30, 0, 55)
        print("[DEVS HUB] UP button released")
    end)
    
    FlyDownButton.MouseButton1Down:Connect(function()
        AndroidDownPressed = true
        FlyDownButton.BackgroundColor3 = Color3.fromRGB(255, 140, 80)
        FlyDownButton.Size = UDim2.new(1, -32, 0, 53)
        print("[DEVS HUB] DOWN button pressed")
    end)
    ...
    FlyDownButton.MouseButton1Up:Connect(function()
        ...
        FlyDownButton.BackgroundColor3 = Color3.fromRGB(255, 100, 50)
        FlyDownButton.Size = UDim2.new(1, -30, 0, 55)
        print("[DEVS HUB] DOWN button released")
    end)
    
    -- Store reference to main frame for show/hide
    FlyUIContainer.MainFrame = MainFrame
    
    print("[DEVS HUB] ✅ Fly UI created successfully!")
    print("[DEVS HUB] UI Container:", FlyUIContainer)
    print("[DEVS HUB] Main Frame:", MainFrame)
    print("[DEVS HUB] Up Button:", FlyUpButton)
    print("[DEVS HUB] Down Button:", FlyDownButton)
end

-- Create Fly UI immediately
if IsAndroid then
    task.spawn(function()
...
local FlightTog = MainTab:AddToggle("Fly Hack", "fly_hack", function(v)
    ...
    if v then
        if IsAndroid then
            -- Show Up/Down buttons on Android
            for _, btn_data in pairs(TouchButtons) do
                if btn_data.Name == "UpBtn" or btn_data.Name == "DownBtn" then
                    btn_data.Button.Visible = true
                end
            end
            Xan.Notify({Title = "Flight", Content = "Toque a tela para se mover, use ▲/▼ para subir/descer", Type = "Success"})
        else
            Xan.Notify({Title = "Flight", Content = "Use WASD + R/Q para voar", Type = "Success"})
        end
    else
        -- Hide Up/Down buttons when Fly is disabled
        if IsAndroid then
            for _, btn_data in pairs(TouchButtons) do
                if btn_data.Name == "UpBtn" or btn_data.Name == "DownBtn" then
                    btn_data.Button.Visible = true
...
MainTab:AddButton("Teleport [U]", function()
    if Config.TargetPlr and Config.TargetPlr.Character and Config.TargetPlr.Character:FindFirstChild("HumanoidRootPart") then
        local success = SafeTeleport(Config.TargetPlr.Character.HumanoidRootPart.CFrame, CFrame.new(0, 0, 4))
        if success then
            Xan.Notify({Title = "Teleport", Content = "Teleported to " .. Config.TargetPlr.Name, Type = "Success"})
        else
            Xan.Notify({Title = "Error", Content = "Teleport failed!", Type = "Error"})
        end
    else
...
PlayerTab:AddButton("Save Position 1", function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        -- Save position with small upward offset to prevent ground clipping
        local currentCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
        Config.SavedPos1 = currentCFrame * CFrame.new(0, 2, 0)
        Xan.Notify({Title = "Position", Content = "Position 1 saved! (Safe height)", Type = "Success"})
    else
...
PlayerTab:AddButton("Save Position 2", function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        -- Save position with small upward offset to prevent ground clipping
        local currentCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
        Config.SavedPos2 = currentCFrame * CFrame.new(0, 2, 0)
        Xan.Notify({Title = "Position", Content = "Position 2 saved! (Safe height)", Type = "Success"})
    else
        Xan.Notify({Title = "Position", Content = "Character not found!", Type = "Error"})
    end
end)
...
PlayerTab:AddButton("Teleport to Position 1", function()
    if Config.SavedPos1 and LocalPlayer.Character then
        local success = SafeTeleport(Config.SavedPos1)
        if success then
            Xan.Notify({Title = "Teleport", Content = "Teleported to Position 1", Type = "Success"})
        else
            Xan.Notify({Title = "Teleport", Content = "Teleport failed!", Type = "Error"})
        end
    else
...
PlayerTab:AddButton("Teleport to Position 2", function()
    if Config.SavedPos2 and LocalPlayer.Character then
        local success = SafeTeleport(Config.SavedPos2)
        if success then
...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    --// FLY HACK (Completely reworked for Android)
    if Config.Fly then
        ...
        -- Show Fly UI on Android
        if IsAndroid and FlyUIContainer and FlyUIContainer.MainFrame then
            FlyUIContainer.MainFrame.Visible = true
            print("[DEVS HUB] ✈️ Fly UI shown (Fly enabled)")
        end
        ...
        if not IsAndroid then
            -- DESKTOP: Standard WASD + E/Q controls
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then
                horizontalDirection = horizontalDirection + Camera.CFrame.LookVector
            end
            ...
            -- ANDROID: Multiple input detection methods for maximum compatibility
            
            -- METHOD 1: Try GetMoveVector() first (most reliable)
            local moveVector = hum:GetMoveVector()
            local forwardInput = 0
            local detectionMethod = "None"
            
            if moveVector.Magnitude > 0.1 then
                forwardInput = moveVector.Z
                detectionMethod = "GetMoveVector"
            else
                -- METHOD 2: Try MoveDirection as fallback
                local moveDir = hum.MoveDirection
                if moveDir.Magnitude > 0.1 then
                    forwardInput = moveDir.Z
                    detectionMethod = "MoveDirection"
                else
                    -- METHOD 3: Direct thumbstick detection
                    local thumbstick1 = UserInputService:GetGamepadState(Enum.UserInputType.Gamepad1)
                    for _, input in pairs(thumbstick1) do
                        if input.KeyCode == Enum.KeyCode.Thumbstick1 then
                            local position = input.Position
                            forwardInput = -position.Y -- Negative Y is forward on thumbstick
                            if math.abs(forwardInput) > 0.1 then
                                detectionMethod = "Thumbstick"
                            end
                            break
                        end
                    end
            ...
            -- Update UI indicator
            if FlyUIContainer and FlyUIContainer.MainFrame then
                local joystickLabel = FlyUIContainer.MainFrame:FindFirstChild("JoystickStatus")
                if joystickLabel then
                    joystickLabel.Text = string.format("Joy: %.2f [%s]", forwardInput, detectionMethod)
                    joystickLabel.TextColor3 = math.abs(forwardInput) > 0.1 
            ...
            if math.abs(forwardInput) > 0.1 then -- Deadzone
                -- Get camera's look direction
                local lookVector = Camera.CFrame.LookVector
                -- Flatten to horizontal plane (remove Y component for pure horizontal movement)
                local horizontalLook = Vector3.new(lookVector.X, 0, lookVector.Z)
                
                if horizontalLook.Magnitude > 0 then
                    horizontalLook = horizontalLook.Unit
                    horizontalDirection = horizontalLook * forwardInput
                end
            end
            
            -- VERTICAL MOVEMENT: Only from UP/DOWN buttons
            if AndroidUpPressed then
                verticalDirection = Vector3.new(0, 1, 0)
            elseif AndroidDownPressed then
                verticalDirection = Vector3.new(0, -1, 0)
            end
        end
        ...
        -- Apply horizontal movement (on XZ plane only)
        if horizontalDirection.Magnitude > 0 then
            -- Flatten to XZ plane (remove any Y component)
            local flatHorizontal = Vector3.new(horizontalDirection.X, 0, horizontalDirection.Z)
            if flatHorizontal.Magnitude > 0 then
                finalVelocity = finalVelocity + (flatHorizontal.Unit * Config.FlySpeed)
            end
        end
        
        -- Apply vertical movement (Y axis only)
        if verticalDirection.Magnitude > 0 then
            finalVelocity = finalVelocity + (verticalDirection * Config.FlySpeed)
        end
        
        -- Set the velocity
        BodyVel.Velocity = finalVelocity
    else
        -- Hide Fly UI on Android when disabled
        if IsAndroid and FlyUIContainer and FlyUIContainer.MainFrame then
            FlyUIContainer.MainFrame.Visible = false
            print("[DEVS HUB] ❌ Fly UI hidden (Fly disabled)")
        end
...
Connections["InfJump"] = UserInputService.JumpRequest:Connect(function()
    ...
    -- Mobile Fly ascend: use jump input to briefly ascend while flying
    if IsAndroid and Config.Fly then
        AndroidJumpPressed = true
        task.delay(0.25, function()
            AndroidJumpPressed = false
        end)
    end
end)
...
Connections["Inputs"] = UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.U and Config.TargetPlr and Config.TargetPlr.Character then
        pcall(function()
            if Config.TargetPlr.Character:FindFirstChild("HumanoidRootPart") then
                local success = SafeTeleport(Config.TargetPlr.Character.HumanoidRootPart.CFrame, CFrame.new(0, 0, 4))
                if success then
                    Xan.Notify({Title = "Teleport", Content = "To: " .. Config.TargetPlr.Name, Type = "Success"})
                end
            end
        end)
    end
    ...
    for _, btn_data in pairs(TouchButtons) do
        ...
        if name == "FlyBtn" then
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                if IsAndroid and FlyUIContainer and FlyUIContainer.MainFrame then
                    FlyUIContainer.MainFrame.Visible = Config.Fly
                    print("[DEVS HUB] Fly button clicked - UI visibility:", Config.Fly)
                end
